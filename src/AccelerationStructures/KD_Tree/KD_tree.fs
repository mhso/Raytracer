...
let rec qsort (xs:list<BBox>) axis =
  match xs with
  | [] -> []
  | x :: xs -> if axis = 0 then      let smaller = qsort (xs |> List.filter(fun e -> e.maxXYZ.x <= x.maxXYZ.x)) axis
                                     let larger  = qsort (xs |> List.filter(fun e -> e.maxXYZ.x >  x.maxXYZ.x)) axis
                                     smaller @ [x] @ larger
               else if axis = 1 then let smaller = qsort (xs |> List.filter(fun e -> e.maxXYZ.y <= x.maxXYZ.y)) axis
                                     let larger  = qsort (xs |> List.filter(fun e -> e.maxXYZ.y >  x.maxXYZ.y)) axis
                                     smaller @ [x] @ larger
               else                  let smaller = qsort (xs |> List.filter(fun e -> e.maxXYZ.z <= x.maxXYZ.z)) axis
                                     let larger  = qsort (xs |> List.filter(fun e -> e.maxXYZ.z >  x.maxXYZ.z)) axis
                                     smaller @ [x] @ larger
...
let findMaxMin (xs:list<BBox>) axis = 
    ...
    | x::xs -> let rec find (xs:list<BBox>) (max:float) (min:float) (axis:int) =
                   match xs with
                   | []    -> (max, min)
                   | x::xs -> if axis = 0 then if x.maxXYZ.x > max && x.minXYZ.x < min then find xs x.maxXYZ.x x.minXYZ.x axis
                              ...
                              if axis = 1 then if x.maxXYZ.y > max && x.minXYZ.y < min then find xs x.maxXYZ.y x.minXYZ.y axis
                                               else if x.maxXYZ.y > max then find xs x.maxXYZ.y min axis
                                               else if x.minXYZ.y < min then find xs max x.minXYZ.y axis
                                               else find xs max min axis
                              else             if x.maxXYZ.z > max && x.minXYZ.z < min then find xs x.maxXYZ.z x.minXYZ.z axis
...
let rec traverseKDTree tree = failwith "Not Implemented"
