...
let main _ = 
    ...
    let boxTexture1 = Textures.mkMatTexture(MatteMaterial(Colour.Red))
    let boxTexture2 = Textures.mkMatTexture(MatteMaterial(Colour.Green))
    let boxTexture3 = Textures.mkMatTexture(MatteMaterial(Colour.White))
    let boxTexture4 = Textures.mkMatTexture(MatteMaterial(Colour.Blue))
    let fig5 = Box(Point(0.0,0.0,0.0), Point(1.0,1.0,1.0), boxTexture1, boxTexture1, boxTexture1, boxTexture1, boxTexture1, boxTexture1)
    let fig6 = Box(Point(0.0,0.0,0.0), Point(3.0,3.0,3.0), boxTexture2, boxTexture2, boxTexture2, boxTexture2, boxTexture2, boxTexture2)
    let fig7 = Box(Point(0.0,0.0,0.0), Point(5.0,5.0,5.0), boxTexture3, boxTexture3, boxTexture3, boxTexture3, boxTexture3, boxTexture3)
    let fig8 = Box(Point(0.0,0.0,0.0), Point(4.0,2.0,3.0), boxTexture4, boxTexture4, boxTexture4, boxTexture4, boxTexture4, boxTexture4)
    let fig6T = transform fig6 (translate 1. 0. 0.)
    let fig7T = transform fig7 (translate 2. 2. 0.)
    let fig8T = transform fig8 (translate -4. -4. 4.)
    let testBoxArr:array<Shape> = [|fig5; fig6T; fig7T; fig8T|]
    ...
    let scene                   = Scene(testBoxArr, camera, lights, lightAmbient, 8)
