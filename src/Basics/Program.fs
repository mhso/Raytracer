...
let main _ = 
    ...
    let matRedTex = Textures.mkMatTexture matteRed

    let sphereRed        = SphereShape(Point(0.,0.,0.), 0.5, matRedTex)
    let transSphere = transform sphereRed (Transformation.translate 0. 1. 0.)
    ...
    let scene                   = Scene([transSphere], lights, lightAmbient, 100)
