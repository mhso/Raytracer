...
module AltRender =
  ...
  type AltRender(scene:Scene, camera:Camera) =
        ...
        if hitPoint.DidHit then
            ...
            scene.Lights 
                |> List.fold (fun accColour light -> 
                    ...
                    let colour = this.CastRecursively ray hitPoint.Shape hitPoint light Colour.Black scene.MaxBounces hitPoint.Material.BounceMethod
    ...
    member this.GetFirstHitPoint (ray:Ray) : HitPoint = 
        ...
        // Get all hit points
        let pointsThatHit = 
            [for s in scene.Shapes do yield s.hitFunction ray]
  ...
  let mkAltRender (scene: Scene) (camera : Camera) : AltRender = AltRender(scene, camera)
