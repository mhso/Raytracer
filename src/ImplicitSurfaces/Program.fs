...
module Program =
  ...
  let main _ = 
    ...
    let sphere1 (r : float) =
      ...
      let s = [|mkShape (mkImplicit ("x^2 + (y + 3)^2 + z^2 - " + (string (r * r)))) (texfun (SpecularMaterial (0.5, aqua, 0.7, white)));
               mkShape (mkImplicit ("(x + 3)^2 + (y - 3)^2 + z^2 - " + (string (r * r)))) (texfun (SpecularMaterial (0.5, Colour.Green, 0.7, white)));
               mkShape (mkImplicit ("(x - 3)^2 + (y + 3)^2 + z^2 - " + (string (r * r)))) (texfun (SpecularMaterial (0.5, Colour.Red, 0.7, white)));
               (InfinitePlane (texfun perfectWhite) :> Shape)
               //mkShape (implicitPlane "y") (texfun phongGreen)
               |]
      let camera = PinholeCamera (Point(0.0, 1.0, 6.0), Point(0.0, 0.0, 0.0), Vector(0.0, 1.0, 0.0), 2.0, 4.0, 3.0, 1024, 768)
      mkScene' s camera
